...
module.exports.unfollowUser = async(req, res) =>{
    ...
    try {
        ...
        if(!user.following.includes(userToUnfollowId)){
            return res.status(400).json({message:"you have not followed this user yet."})
        }

        userToUnfollow.follower= userToUnfollow.follower.filter((id) => id.toString() !== userId);
...
module.exports.followerCount = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({message:"User follower count", followerCount: followerCount});
    } catch (error) {
...
module.exports.followingCount = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({message:"User follower count", followingCount: followingCount});
    } catch (error) {
        console.error("Error in getting followerCount : ", error);
        return res.status(500).json({message:"Internal server error."})
    }
